...
class SemiModel():
    ...
    def predict_cam_using_path(self,img_path):

        result_set = {"prob":'', "superimposed_img":''}

        #predict
        pre_img = self.preprocessing_on_path(img_path)
        img = np.reshape(pre_img, (-1, self.IMG_HEIGHT, self.IMG_WIDTH, 3))
        with self.graph.as_default():
          predict_value = self.model.predict(img)
          prob = np.asscalar(predict_value)
        ...
        last_conv_layer = self.model.get_layer(index = -5)
        # x = self.preprocessing_on_path(img_path)
        x = np.reshape(pre_img, (1, self.IMG_HEIGHT, self.IMG_WIDTH, 3))
        with self.graph.as_default():
            grads = K.gradients(self.model.output, last_conv_layer.output)
            ...
            superimposed_img = heatmap * hif + img
        #set result data    
        result_set["prob"]=prob
        result_set["superimposed_img"]=superimposed_img
